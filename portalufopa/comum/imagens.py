...
@permission_content(tipo='ATImagem', permissao='create', login_url='/security/login/')
def create(request):
...
@permission_content(tipo='ATImagem', permissao='update', login_url='/security/login/')
def edit(request):
    _url = reescrever_url(request)
    _site_url = get_site_url_id(request)
    _content_url = get_url_id_content(request)
    _object = Imagem.objects.filter(site__url=_site_url).get(url=_content_url)
    form = ImagemForm(request.POST or None, request.FILES or None, instance=_object)
    if form.is_valid():
        model = form.save(commit=False)
...
@permission_content(tipo='ATImagem', permissao='delete', login_url='/security/login/')
...
@permission_content(tipo='ATImagem', permissao='workflow', login_url='/security/login/')
